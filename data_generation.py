...
def generate_potential_data(num_samples, dx, dy, max_x_extent, max_y_extent, potential_type, min_x_extent=2, min_y_extent=2, lambda_value=0.1, num_eigenvalues=2):
    potentials = []
    eigenvalues = []
    ...
    for _ in range(num_samples):
        ...
        if potential_type == "harmonic":
            ...
            well_width_x = np.random.uniform(min_x_extent, max_x_extent)  # Random width in X direction for the rectangular well
            well_width_y = np.random.uniform(min_y_extent, max_y_extent)  # Random width in Y direction for the rectangular well
            V[(np.abs(X) <= well_width_x / 2) & (np.abs(Y) <= well_width_y / 2)] = well_depth
        else:
            raise ValueError("Unknown potential type: choose 'harmonic', 'anharmonic', or 'well'")

        # Generate finite difference matrices for the potential grid
        D2_x = sp.diags([1, -2, 1], [-1, 0, 1], shape=(grid_size_x, grid_size_x)) / dx**2
        D2_y = sp.diags([1, -2, 1], [-1, 0, 1], shape=(grid_size_y, grid_size_y)) / dy**2
        ...
        try:
            ...
            if num_eigenvalues >= N:
                ...
                E, _ = spla.eigsh(H, k=num_eigenvalues, which='SM', maxiter=50000, tol=1e-4)
                E = np.sort(E)
    ...
    potentials = torch.tensor(potentials).float()
